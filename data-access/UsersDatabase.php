...
class UsersDatabase extends Database
{
    ...
    {
        $query = "INSERT INTO users (username, password_hash, location, premium) VALUES (?, ?, ?, ?)";

        $stmt = $this->conn->prepare($query);

        $stmt->bind_param("sssi", $user->username, $user->password_hash, $user->location, $user->premium);
    ...
    {
        $query = "UPDATE users SET username=?, location=?, premium=? WHERE user_id=?;";

        $stmt = $this->conn->prepare($query);

        $stmt->bind_param("ssii", $user->username, $user->location, $user->premium, $user_id);
